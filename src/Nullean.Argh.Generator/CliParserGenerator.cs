...
public sealed class CliParserGenerator : IIncrementalGenerator
{
	...
	private static void Execute(SourceProductionContext context, Compilation compilation, ImmutableArray<InvocationExpressionSyntax> invocations)
	{
		...
		foreach (InvocationExpressionSyntax invocation in invocations)
		{
			...
			if (receiver is null || !SymbolEqualityComparer.Default.Equals(receiver, arghApp))
				continue;
		...
		if (!TryBuildAppEmitModel(context, compilation, filtered, arghApp, out AppEmitModel? appModel) || appModel is null)
		{
			EmitEmpty(context);
			return;
		}
	...
	private static void EmitRootCompletionsBlock(StringBuilder sb, string indent)
	{
		sb.AppendLine(indent + "if (args.Length >= 2 && args[0] == \"--completions\")");
		sb.AppendLine(indent + "{");
		sb.AppendLine(indent + "\tvar __shell = args[1];");
		sb.AppendLine(indent + "\tvar __entry = Assembly.GetEntryAssembly()?.GetName().Name ?? \"app\";");
	...
	private static void EmitFuzzyFlatSwitchDefault(StringBuilder sb, ImmutableArray<CommandModel> commands)
	{
		List<CommandModel> sorted = commands.OrderBy(c => c.CommandName, StringComparer.Ordinal).ToList();
		sb.AppendLine("\t\t\t\tvar __tok = args[0];");
		sb.AppendLine("\t\t\t\tvar __app = Assembly.GetEntryAssembly()?.GetName().Name ?? \"app\";");
	...
	private static void EmitFlat(SourceProductionContext context, AppEmitModel app)
	{
		...
		sb.AppendLine("\t\t\tvar ct = cts.Token;");
		sb.AppendLine("\t\t\ttry");
		sb.AppendLine("\t\t\t{");
		...
		EmitFuzzyFlatSwitchDefault(sb, commands);
		sb.AppendLine("\t\t\t}");
		sb.AppendLine("\t\t}");
		sb.AppendLine();
		...
		sb.AppendLine("\t\t{");
		sb.AppendLine("\t\t\tvar v = Assembly.GetEntryAssembly()?.GetName().Version;");
		sb.AppendLine("\t\t\tConsole.Out.WriteLine(v?.ToString() ?? \"0.0.0.0\");");
		sb.AppendLine("\t\t}");
		sb.AppendLine();
	...
	private static void EmitHierarchical(SourceProductionContext context, AppEmitModel app)
	{
		...
		sb.AppendLine("\t\t}");
		sb.AppendLine();
		EmitRunCoreHierarchical(sb, app);
		sb.AppendLine();
		...
		EmitDispatchForNode(sb, app, app.Root, ImmutableArray<string>.Empty, "DispatchRoot", isRoot: true);
		sb.AppendLine("\t\tprivate static bool? ParseNullableBool(string? raw, bool fromYesSwitch)");
		sb.AppendLine("\t\t{");
		sb.AppendLine("\t\t\tif (string.IsNullOrEmpty(raw)) return fromYesSwitch;");
		sb.AppendLine("\t\t\tif (bool.TryParse(raw, out var b)) return b;");
	...
	private static void EmitRunCoreHierarchical(StringBuilder sb, AppEmitModel app)
	{
		...
		bool isRoot)
	{
		...
		sb.AppendLine("\t\t\t{");
		EmitFuzzyDispatchDefault(sb, node, path);
		sb.AppendLine("\t\t\t}");
		sb.AppendLine("\t\t}");
		sb.AppendLine();
		foreach (RegistryNode.NamedGroupChild ch in node.Children)
		{
			ImmutableArray<string> childPath = AppendSegment(path, ch.Segment);
			EmitDispatchForNode(sb, app, ch.Node, childPath, "DispatchGroup_" + GroupPathKey(childPath), isRoot: false);
		}
	}
	...
	private static void EmitCommandRunner(StringBuilder sb, CommandModel cmd, ImmutableArray<GlobalFilterRegistration> globalFilters)
	{
		...
		else
		{
			...
			for (int i = cmd.CommandFilters.Length - 1; i >= 0; i--)
			{
				...
				sb.AppendLine($"\t\t\tnext = async c => await new {fq}().InvokeAsync(c, {name});");
			}
			...
			for (int i = globalFilters.Length - 1; i >= 0; i--)
			{
				...
				string name = "__cap" + cap++;
				sb.AppendLine($"\t\t\tvar {name} = next;");
				sb.AppendLine($"\t\t\tnext = async c => await new {gFq}().InvokeAsync(c, {name});");
			}
		...
		string lineIndent = "\t\t\t")
	{
		...
		string call = cmd.RequiresInstance
			? $"new {cmd.ContainingTypeFq}().{cmd.MethodName}({argList})"
